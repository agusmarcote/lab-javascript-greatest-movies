...
function getAllDirectors(moviesArray) {
    let movieDirector = moviesArray.map(directors =>{
...
function howManyMovies(moviesArray) {
    ...
    let stevenSpielberg = moviesArray.filter(i =>{
        return i.director === 'Steven Spielberg' && i.genre.includes('Drama')
        })
...
function scoresAverage(moviesArray) {
    ...
    let scores = moviesArray.map(i =>{
        return i.score
    })
    let puntuaciones = scores.reduce(function(accumulator, currentValue){
        if (currentValue){
    ...
    let avgScore = (puntuaciones / scores.length)
...
function dramaMoviesScore(moviesArray) {
    let dramaticMovies = moviesArray.filter(i =>{
        return i.genre.includes('Drama')
    })
    if (dramaticMovies == 0){
      return 0
    }
    let avgDrama = dramaticMovies.reduce(function(accumulator, currentValue){
      return accumulator + currentValue.score
    }, 0)

    let result = Number((avgDrama / dramaticMovies.length).toFixed(2))   
...
function orderByYear(moviesArray) {
  let wholeYear = moviesArray.map(i=> {
    return i
  }) 

  wholeYear.sort(function(a, b){
...
function orderAlphabetically(moviesArray) {
  let titleList = moviesArray.map(i=> {
    return i.title
  })
  titleList.sort()
...
function turnHoursToMinutes(moviesArray) {
  let timeMinutes = moviesArray.map(i=>{
    if (i.duration.length <= 2){
      i.duration = Number(i.duration[0]) * 60
      return i
    }

    let hourSplit = i.duration.split('h ')
    let minSplit =hourSplit[1].split('min')
    let result = parseInt(Number(hourSplit[0])) * 60 + parseInt(Number(minSplit[0]));
    i.duration = result
